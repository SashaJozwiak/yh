    ...
    return (
        <div className={s.footer}>
            ...
            <button
                ...
                className={`${s.btn} ${nav === 'UHS' ? s.btnOn : /* nav === 'UHS' ? s.btnOn : */ null}`}>
            ...
            <button
                disabled={!isAuth}
                style={{ opacity: !isAuth ? '0.5' : '1', border: '1px solid gray' }}

                onClick={() => changeNav('game1')} className={`${s.btn} ${nav === 'game' ? s.btnOn : null}`}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke={isAuth ? 'white' : 'gray'} width={'2rem'} className="size-6">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5.25 5.653c0-.856.917-1.398 1.667-.986l11.54 6.347a1.125 1.125 0 0 1 0 1.972l-11.54 6.347a1.125 1.125 0 0 1-1.667-.986V5.653Z" />
                </svg>
                <p style={{ color: 'white', opacity: isAuth ? '1' : '0.5' }}>GAME</p>
            ...
            <button
                disabled={!isAuth}
                style={{ opacity: !isAuth ? '0.5' : '1' }}
                onClick={() => changeNav('invite')} className={`${s.btn} ${nav === 'invite' ? s.btnOn : null}`}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" width={'2rem'} className="size-6">
            ...
            <button
                disabled={!tradeReady} //change after acces to balance
                style={{ opacity: tradeReady ? '1' : '0.5' }} //change after acces to balance
                onClick={() => changeNav('stage')}
                className={`${s.btn} ${nav === 'stage' ? s.btnOn : null}`}>
                {/* <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} width={'2rem'} stroke="currentColor" className="size-6">
